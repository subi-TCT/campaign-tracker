...
// ONE-TIME SYNC database route
app.get('/api/run-one-time-sync', async (req, res) => {
  try {
    let excelPath = path.join(__dirname, 'IAS_All_Kerala_Districts.xlsx');
    if (!fs.existsSync(excelPath)) {
      excelPath = path.join(__dirname, '../IAS_All_Kerala_Districts.xlsx');
    }
    
    if (!fs.existsSync(excelPath)) {
      return res.status(404).json({ error: 'Spreadsheet IAS_All_Kerala_Districts.xlsx not found inside container.' });
    }
    
    console.log('Reading Excel data inside container...');
    const workbook = xlsx.readFile(excelPath);
    const sheet = workbook.Sheets['All Kerala Districts'];
    if (!sheet) {
      return res.status(404).json({ error: 'Sheet "All Kerala Districts" not found in Excel.' });
    }
    
    const rows = xlsx.utils.sheet_to_json(sheet, { range: 0 });
    const excelCodes = new Set();
    for (const row of rows) {
      const memCodeRaw = row['Mem#'];
      if (memCodeRaw) {
        excelCodes.add(String(memCodeRaw).trim().replace(/\s+/g, ''));
      }
    }
    const codesArray = Array.from(excelCodes);
    
    console.log('Starting DB update inside container...');
    let dbUpdatedCount = 0;
    
    // 1. Update District and Area (in parallel chunks to prevent timeout)
    const batchSize = 50;
    for (let i = 0; i < rows.length; i += batchSize) {
      const chunk = rows.slice(i, i + batchSize);
      await Promise.all(chunk.map(async (row) => {
        const memCodeRaw = row['Mem#'];
        if (!memCodeRaw) return;
        
        const accCode = String(memCodeRaw).trim().replace(/\s+/g, '');
        const district = String(row['DISTRICT'] || '').trim();
        const area = String(row['AREA'] || '').trim();
        
        const updateRes = await run(
          "UPDATE contacts SET district = ?, area = ? WHERE acc_code = ?",
          [district, area || null, accCode]
        );
        dbUpdatedCount += updateRes.changes;
      }));
    }
    
    // 2. Set all to Inactive
    await run("UPDATE contacts SET account_status = 'Inactive'");
    
    // 3. Set matching ones to Active
    let dbActivatedCount = 0;
    for (let i = 0; i < codesArray.length; i += 100) {
      const batch = codesArray.slice(i, i + 100);
      const placeholders = batch.map(() => '?').join(',');
      const activateRes = await run(
        `UPDATE contacts SET account_status = 'Active' WHERE acc_code IN (${placeholders})`,
        batch
      );
      dbActivatedCount += activateRes.changes;
    }
    
    res.json({
      success: true,
      excelRowsLoaded: rows.length,
      uniqueMemberCodes: codesArray.length,
      recordsUpdated: dbUpdatedCount,
      recordsActivated: dbActivatedCount,
      recordsInactive: (rows.length - dbActivatedCount)
    });
  } catch (err) {
    console.error('Error executing one-time sync inside container:', err.message);
    res.status(500).json({ error: err.message });
  }
});
